<?php
  ...
  function test_input($data) {
	...
	$sql = mysqli_query($dbsiakad,"select * from masterdosen where dos_kode='$username' and pin='$password'");
	$cek = mysqli_num_rows($sql);
	if($cek > 0){
		$row = mysqli_fetch_array($sql);
			$nip = $row['DOS_NIP'];
			$gelardepan = $row['DOS_GELARDEPAN'];
			$namadosen = $row['DOS_NAMA'];
			$gelarbelakang = $row['DOS_GELARBELAKANG'];
			$nama = $gelardepan.$namadosen.$gelarbelakang;
		...
		if($cek > 0){
			...
			//yg login mahasiswa
			$sql = mysqli_query($dbsiakad, "select * from mastermahasiswa where nim='$username' and pin='$password'");
			$cek = mysqli_num_rows($sql);
			if ($cek > 0){
				$data = mysqli_fetch_array($sql);
					$iduser = $data['NIM'];
