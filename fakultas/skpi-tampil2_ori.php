									...
									<table class="table table-striped">
										...
										<tbody>
											<?php
												...
												$qprestasi = mysqli_query($dbsurat,"SELECT * FROM skpi WHERE nim='$nim' AND verifikator3='$iduser' AND verifikasi3=0 AND verifikasi2=1 ORDER BY cpl, indonesia");
												while($data = mysqli_fetch_array($qprestasi))
												{ $nodata = $data[0];
											?>
											<tr>
												<form role="form" method="POST">
													...
													<td>
														...
														<input type="hidden" class="form-control" name="nodata" value="<?= $nodata;?>"/>
														<button name ="aksi" value="tolak" type="submit" onclick="return confirm('Menolak CPL <?= $data['indonesia'];?> ?')" formaction="skpi-cpl-tolak2.php" class="btn btn-sm btn-danger"> <i class="fa fa-times"></i> </button>
													</td>
												</form>
											</tr>
											<?php
