            ...
            <div class="card-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
								...
								<!-- ijin lab -->
								<?php
									$query = mysqli_query($dbsurat, "SELECT * FROM ijinlab WHERE prodi = '$jurusan' ORDER BY tglmulai desc");
									while($data = mysqli_fetch_array($query)){
										$nodata = $data['no'];
										$nim = $data['nim'];
										$nama = $data['nama'];
										$namalab = $data['namalab'];
										$tglmulai = tgl_indo($data['tglmulai']);
										$tglselesai = tgl_indo($data['tglselesai']);
										$jurusan = $data['prodi'];
